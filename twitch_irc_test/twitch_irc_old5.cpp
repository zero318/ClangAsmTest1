...
static union {
    _Atomic(uint8_t) recent_message_lock_atomic = false;
    bool recent_message_lock;
};
...
bool __fastcall lock_recent_messages() {
    if (expect(filled_message_slots != 0, true)) {
        recent_message_lock_atomic = true;
        return true;
    }
    return false;
}
...
void __fastcall unlock_recent_messages() {
    recent_message_lock_atomic = false;
}
...
void parse_twitch_chat(std::stop_token& stoken, const L1& pre_message, const L2& parse) {
    do {
...
static union {
    _Atomic(uint8_t) reset_votes_atomic = false;
    bool reset_votes;
};

static union {
    _Atomic(uint8_t) enable_voting_atomic = false;
    bool enable_voting;
};

static inline constexpr uint32_t valid_votes_default = 0b00001111;
static union {
    _Atomic(uint32_t) valid_votes_atomic;
...
std::array<size_t, 4> __fastcall get_final_votes() {
    ...
    valid_votes_atomic = temp;
    reset_votes_atomic = true;
                        ...
                        [](std::stop_token stoken) {
                            parse_twitch_chat(
                                ...
                                [](const std::string_view& username, const std::string_view& message) {
                                    ...
                                    if (
                                        enable_voting &&
                                        (vote = *(uint16_t*)message.data() - '1') < 8
                                    ) {
