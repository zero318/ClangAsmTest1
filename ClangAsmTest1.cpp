...
// RAX, RCX, RDX, RDI, RSI, R8, R9, R12, R14, R15
uint64_t regcall regcall_arg_test(
	uint64_t RAX,
	uint64_t RCX,
	uint64_t RDX,
	uint64_t RDI,
...
dllexport void find_u16_tangent() {
	for (volatile size_t i = 0; i < 2048; ++i) {
		uint32_t value = finetangent[i];
...
#define FINE_TANGENT_MASK 0xFFF
int32_t fine_tangent(uint32_t angle) {
	angle &= FINE_TANGENT_MASK;
	int32_t x = angle - 2048;
	int32_t mask = x >> 31;
	x ^= mask; // if (angle < 2048) x = -x - 1;
	int32_t ret = finetangent[x];
	return (ret ^ mask) - mask; // if (angle < 2048) ret = -ret;
}
...
struct KEY_VALUE_PARTIAL_INFORMATIONX2 {
	KEY_VALUE_PARTIAL_INFORMATION* operator&() const {
		return (KEY_VALUE_PARTIAL_INFORMATION*)this;
	}
...
constexpr auto wkjrbkwejrbkjbr = sizeof(KEY_VALUE_PARTIAL_INFORMATIONX2<wchar_t[MAX_PATH]>);

template<size_t N>
static inline constexpr STRING InitAsciiString(const char(&str)[N]) {
	return {
		sizeof(wchar_t[N - 1]),
		sizeof(wchar_t[N]),
		str
...
template<size_t N>
static inline constexpr UNICODE_STRING InitUnicodeString(const wchar_t(&str)[N]) {
	return {
		sizeof(wchar_t[N - 1]),
		sizeof(wchar_t[N]),
		str
...
static inline constexpr UNICODE_STRING MakeUnicodeString(const wchar_t* str) {
	if (str) {
		size_t length = 0;
		while (str[length++]);
		if (length > 0x7FFEu) length = 0x7FFEu;
		length *= sizeof(wchar_t);
		return {
			(uint16_t)length,
			(uint16_t)(length + sizeof(wchar_t)),
			(wchar_t*)str
		};
	} else {
		return {};
...
static inline constexpr void InitUnicodeString(UNICODE_STRING* out, const wchar_t* str) {
...
constexpr UNICODE_STRING test_str = L"InstallPath"_wZ;