...
SET "out=F:\Touhou_Stuff_2\_thtk-bin-new\__thcrap\repos\zero318\testing\testing\th18\"
...
for %%g in (*.ecl_dump_map) do (
	...
	%thecl% -c 18 -m "%truth_map_path%th18_thtk.eclm" "%%~ng.ecl_dump_map"  "%out%st01.ecl"
	REM %thecl% -d 18 -m "%truth_map_path%th18_thtk.eclm" "%out%%%~ng.ecl" "%%~ng.ecl_redump_map"
	REM %thecl% -d 18 -r -m "%truth_map_path%th18_thtk.eclm" "%out%%%~ng.ecl" "%%~ng.ecl_raw_redump_map"
    REM COPY "%out%%%~ng.ecl" "%outD%%%~ng.ecl"
)