...
inline size_t _fputs_nolock(const char* str, FILE* file) {
	return _fwrite_nolock(str, strlen(str), 1, file);
}
...
int main(int argc, char* argv[]) {
	...
	if (argc >= 9) {
		if (FILE* globals_file = fopen(argv[2], "r")) {
			...
			_fseek_nolock(globals_file, 0, SEEK_END);
			long globals_length = _ftell_nolock(globals_file);
			_fseek_nolock(globals_file, 0, SEEK_SET);
			if (char* globals = (char*)malloc(globals_length + 1)) {
				globals[globals_length] = '\0';
				_fread_nolock(globals, globals_length, 1, globals_file);
				...
				if (FILE* floats_file = fopen(argv[5], "r")) {
					_fseek_nolock(floats_file, 0, SEEK_END);
					long floats_length = _ftell_nolock(floats_file);
					_fseek_nolock(floats_file, 0, SEEK_SET);

					if (char* floats_str = (char*)malloc(floats_length + 1)) {
						floats_str[floats_length] = '\0';
						_fread_nolock(floats_str, floats_length, 1, floats_file);
						...
						if (FILE* strings_file = fopen(argv[6], "r")) {
							_fseek_nolock(strings_file, 0, SEEK_END);
							long strings_length = _ftell_nolock(strings_file);
							_fseek_nolock(strings_file, 0, SEEK_SET);

							if (char* strings_str = (char*)malloc(strings_length + 1)) {
								strings_str[strings_length] = '\0';
								_fread_nolock(strings_str, strings_length, 1, strings_file);
								...
								*/
								...
								if (FILE* ptr_hacks_file = fopen(argv[8], "r")) {
									_fseek_nolock(ptr_hacks_file, 0, SEEK_END);
									long ptr_hacks_length = _ftell_nolock(ptr_hacks_file);
									_fseek_nolock(ptr_hacks_file, 0, SEEK_SET);

									if (char* ptr_hacks_str = (char*)malloc(ptr_hacks_length + 1)) {
										ptr_hacks_str[ptr_hacks_length] = '\0';
										_fread_nolock(ptr_hacks_str, ptr_hacks_length, 1, ptr_hacks_file);

										std::vector<std::pair<std::string_view, std::string_view>> ptr_hacks;

										char* ptr_hacks_read = ptr_hacks_str;
										while (char* ptr_str_end = strchr(ptr_hacks_read, ',')) {
											char* replace_end = strchr(ptr_str_end + 1, '\n');
											ptr_hacks.emplace_back(std::string_view(ptr_hacks_read, ptr_str_end - ptr_hacks_read), std::string_view(ptr_str_end + 1, replace_end++ - (ptr_str_end + 1)));
											ptr_hacks_read = replace_end;
										}
										
										/*
										for (const auto& [ptr, hack] : ptr_hacks) {
											printf(
												"Mapping hack: %.*s -> %.*s\n"
												, (int)ptr.size(), ptr.data()
												, (int)hack.size(), hack.data()
											);
										}
										*/
																		...
																		if (json_t* instruction = json_object_get(instruction_json, "instruction")) {
																			if (json_t* formatted = json_object_get(instruction, "formatted")) {
																						...
																						if (formatted_instr[5] == '_' || formatted_instr[5] == '@') {
																							...
																							// the _ from left inputs than add it to the right ones.
																								...
																								if (standard_library_names.find(&formatted_instr[5]) != standard_library_names.end()) {
																									_fwrite_nolock("call ", 5, 1, left_output);
																									_fputs_nolock(&formatted_instr[6], left_output);
																									goto printed_symbolL;
																								}
																							}
																						}
																					...
																					if (mangled_start[1] == '?') {
																						if (const char* str_end = strrchr(mangled_start, '@')) {
																							switch (str_end[1]) {
																								case '\0':
																									// Check for vtables
																									if (mangled_start[3] == '7') {
																										break;
																									}
																								case '+': case '-': case ']':
																									_fwrite_nolock(formatted_instr, mangled_start - formatted_instr, 1, left_output);
																									_fwrite_nolock("STRING", 6, 1, left_output);
																									_fputs_nolock(str_end + 1, left_output);
																									goto printed_symbolL;
																							}
																						}
																					}
																					...
																					if (const char* mangled_end = strchr(mangled_start, '@')) {
																						...
																						for (const GlobalData& global : global_vars) {
																							if (global.name == global_var) {
																								...
																								_fputs_nolock(symbol_end, left_output);
																								goto printed_symbolL;
																							}
																						}
																					}
																				}
																				// Switch tables don't match, just replace with LABEL
																				else if (const char* label_sigil = strchr(formatted_instr, '$')) {
																					_fwrite_nolock(formatted_instr, label_sigil - formatted_instr, 1, left_output);
																					_fwrite_nolock("LABEL", 5, 1, left_output);
																					if (strchr(label_sigil, ']')) {
																						_fputc_nolock(']', left_output);
																					}
																					goto printed_symbolL;
																				}
																				...
																				// with 0x0 than it is the other way around.
																					...
																					_fputs_nolock(except_list + 13, left_output);
																					goto printed_symbolL;
																				}
																		...
																		if (json_t* instruction = json_object_get(instruction_json, "instruction")) {
																			if (json_t* formatted = json_object_get(instruction, "formatted")) {
																				...
																				if (!strncmp(formatted_instr, "call", 4)) {
																					...
																					else if (!strncmp(&formatted_instr[5], "FID_conflict:_fwprintf", 22)) {
																						_fwrite_nolock("call fprintf", 12, 1, right_output);
																						goto printed_symbolR;
																					}
																					else if (!strncmp(&formatted_instr[5], "D3D8.DLL::Direct3DCreate8", 25)) {
																						_fwrite_nolock("call Direct3DCreate8", 20, 1, right_output);
																						goto printed_symbolR;
																					}
																					else if (!strncmp(&formatted_instr[5], "DINPUT8.DLL::DirectInput8Create", 31)) {
																						_fwrite_nolock("call DirectInput8Create", 23, 1, right_output);
																						goto printed_symbolR;
																					}
																					else if (!strncmp(&formatted_instr[5], "DSOUND.DLL::DirectSoundCreate8", 30)) {
																						_fwrite_nolock("call DirectSoundCreate8", 23, 1, right_output);
																						goto printed_symbolR;
																					}
																					else if (!strncmp(&formatted_instr[5], "FUN_0045c3e0", 12)) {
																						_fwrite_nolock("call __EH_prolog", 16, 1, right_output);
																						goto printed_symbolR;
																					}
																				...
																				if (const char* dat_str = strstr(formatted_instr, "DAT_")) {
																					...
																					if (matched_global) {
																						...
																						_fputs_nolock(end_str, right_output);
																						goto printed_symbolR;
																					}
																				...
																				// Random hacks to make things match
																				if (const char* ptr_str_start = strstr(formatted_instr, "PTR_")) {
																					const char* ptr_str_end;
																					// funny logic to preserve any offsets that were applied
																					(ptr_str_end = strchr(ptr_str_start, '+')) ||
																					(ptr_str_end = strchr(ptr_str_start, '-')) ||
																					(ptr_str_end = strchr(ptr_str_start, ']')) ||
																					(ptr_str_end = strchr(ptr_str_start, '\0'));
																					std::string_view ptr_str(ptr_str_start, ptr_str_end - ptr_str_start);
																					//printf("Looking for hack:%.*s\n", (int)ptr_str.size(), ptr_str.data());
																					for (const auto& [ptr, hack] : ptr_hacks) {
																						if (ptr_str == ptr) {
																							_fwrite_nolock(formatted_instr, ptr_str_start - formatted_instr, 1, right_output);
																							_fwrite_nolock(hack.data(), hack.size(), 1, right_output);
																							_fputs_nolock(ptr_str_end, right_output);
																							goto printed_symbolR;
																						}
																					}
																				}
																				// Switch tables don't match, just replace with LABEL
																				if (const char* label_str;
																					(label_str = strstr(formatted_instr, "switch")) ||
																					(label_str = strstr(formatted_instr, "LAB_"))
																				) {
																					_fwrite_nolock(formatted_instr, label_str - formatted_instr, 1, right_output);
																					_fwrite_nolock("LABEL", 5, 1, right_output);
																					if (strchr(label_str, ']')) {
																						_fputc_nolock(']', right_output);
																					}
																					goto printed_symbolR;
