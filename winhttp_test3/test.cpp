...
static int tls_write(tls_socket* s, const void* buffer, int size) {
    while (size != 0)
    {
        int use = __min(size, s->sizes.cbMaximumMessage);
...
static int tls_read(tls_socket* s, void* buffer, int size) {
    ...
    while (size != 0)
    {
        if (s->decrypted)
        {
            ...
            int use = __min(size, s->available);
...
int main(int argc, char* argv[]) {
    const char* hostname = "ns.photonengine.io";
    ...
    //tls_socket s;
    tls_socket* s = (tls_socket*)__builtin_alloca(sizeof(tls_socket));
    //if (tls_connect(s, hostname, 443) != 0)
    if (tls_connect(s, hostname, 19093) != 0)
    {
