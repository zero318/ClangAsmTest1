...
.macro nop_align mult
.align \mult
.endm
.macro nop_align_offset mult start stop
.nops ((.+(\stop-\start)+(\mult-1))&-\mult)-.-(\stop-\start)
.endm
.macro int3_align mult
.align \mult, 0xCC
.endm
.macro int3_align_offset mult start stop
.skip ((.+(\stop-\start)+(\mult-1))&-\mult)-.-(\stop-\start), 0xCC
.endm
...
no_resize:
	...
	ADD ESP, 0xC
	ADD EDI, EAX
	MOV DWORD PTR [EBP+0x740], EDI
	ADD EBX, EAX
	MOV DWORD PTR [EBP+0x75C], EBX
	ADD ESI, EBX
	MOV DWORD PTR [EBP+0x760], ESI
	MOV ECX, DWORD PTR [EBP+0x724]
	JMP start_world_preprocess

int3_align_offset 16, world_layer_table_set, world_layer_set
...
int3_align_offset 16, world2_layer_table_set, world2_layer_set
...
th19a_render_layer_turbo:
	...
	SUB ESP, 8
	MOV EBP, ECX
	MOV ESI, DWORD PTR [ECX+0x73C]
	MOV EDI, DWORD PTR [ECX+0x740]
	CMP ESI, EDI
	JE skip_side_0
	...
	MOV ECX, EBP
skip_side_0:
	MOV ESI, DWORD PTR [ECX+0x75C]
	MOV EDI, DWORD PTR [ECX+0x760]
	CMP ESI, EDI
	JE skip_side_1
	IMUL EBX, EDX, 0x01010101
	PUSH 1
