...
struct socket_data {
	...
	size_t mappings_len;
	size_t mappings_cap;
	transient_peer* transient_peers;
...
socket_data* get_socket_data(SOCKET s) {
...
int WSAAPI my_WSARecvFrom(
  ...
  LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine
	...
	socket_data* socket_data = get_socket_data(s);
	if (!socket_data) {
...
int WSAAPI my_bind(SOCKET s, const sockaddr* name, int namelen) {
	if (name) {
		log_printf(
			"starting bind: socket=%zu, family=%d, address=%s\n"
			, s, name->sa_family, inet_ntoa(((const sockaddr_in*)name)->sin_addr)
		);
	} else {
		log_printf(
			"starting bind: socket=%zu without address\n"
			, s
		);
	}
