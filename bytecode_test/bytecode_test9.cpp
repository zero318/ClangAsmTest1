...
struct HydraVM {
    ...
    private:
        struct StackA {
            ...
            constexpr inline ST& operator[](intptr_t index) const {
        ...
        public:
            ...
            constexpr inline void swap(T& value, size_t index = 0) {
                value = this->template exchange<T, T>(value, index);
            }
    ...
    private: 
        ...
        struct InstrB {
            ...
            constexpr inline InstrB& operator=(size_t index) {
                this->index = index;
                return *this;
            }
    ...
    public:
        ...
        alignas(64) static constexpr dispatch_instr_impl *const dispatch_table[MAX_OPCODE] = {
            ...
            [JMP] = [](InstrPtr ip, Stack stack, PI itop, FL ftop) static constexpr preserve_none {
                return dispatch_direct(ip + ip->argi, stack, itop, ftop);
            },
            [JZ] = [](InstrPtr ip, Stack stack, PI itop, FL ftop) static constexpr preserve_none {
                intptr_t offset = stack.pop() ? 1 : ip->argi;
...
#define PRINTITOP_END {.opcode = PRINT,.arg='i'},{.opcode=ENDL},
#define PRINTFTOP_END {.opcode = FPRINT,.arg='f'},{.opcode=ENDL},
...
int fibonacci(int index) {
    ...
    return HydraVM::start_dispatching_with_values(instructions, index, 0.0f);
}
